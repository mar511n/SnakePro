...
@onready var labeledHSlider = preload("res://scenes/labeled_h_slider.tscn")
@onready var labeledButton = preload("res://scenes/labeled_button.tscn")

const pl_set_g = "PlayerModuleSetting"
const pl_set_mbg = "PlayerModuleSettingCheckBtn"
const g_set_g = "GameModuleSetting"
const g_set_mbg = "GameModuleSettingCheckBtn"

@onready var scrollCont = $ScrollContainer
@onready var vflow = $ScrollContainer/VFlowContainer
@onready var startSnakeLength = $ScrollContainer/VFlowContainer/startSnakeLength
@onready var snakeSpeed = $ScrollContainer/VFlowContainer/snakeSpeed
@onready var mapPaths_b = $ScrollContainer/VFlowContainer/mapPaths_b

var playerModuleProps = {}
var gameModuleProps = {}
...
func make_game_module_settings():
	clear_game_modules()
...
func set_game_module_settings():
	var mods = Global.config_get_section_dict(Global.config_game_mods_sec)
	var properties = Global.config_get_section_dict(Global.config_game_mod_props_sec)
	for mod in mods:
		if gameModuleProps.has(mod) and mods[mod] is Array and len(mods[mod]) > 1:
			gameModuleProps[mod]["btn"].button_pressed = mods[mod][1]
			for prop in gameModuleProps[mod]["props"]:
				if properties.has(prop):
					gameModuleProps[mod]["props"][prop].set_prop_value(properties[prop])

func on_save_gamemodule_properties():
	var mods = {}
	var properties = {}
	for mod in gameModuleProps:
...
func add_game_module(path:String, mname:String, properties:Dictionary):
	...
	for prop in properties:
		if properties[prop][0] is bool:
			var propB = labeledButton.instantiate()
			propB.text = prop
			...
			if properties[prop][0] is float or properties[prop][0] is int:
				var propSl = labeledHSlider.instantiate()
...
func make_player_module_settings():
...
func set_player_module_settings():
	var mods = Global.config_get_section_dict(Global.config_player_mods_sec)
	var properties = Global.config_get_section_dict(Global.config_player_mod_props_sec)
	for mod in mods:
		if playerModuleProps.has(mod) and mods[mod] is Array and len(mods[mod]) > 1:
			playerModuleProps[mod]["btn"].button_pressed = mods[mod][1]
			for prop in playerModuleProps[mod]["props"]:
				if properties.has(prop):
					playerModuleProps[mod]["props"][prop].set_prop_value(properties[prop])

func on_save_playermodule_properties():
	var mods = {}
	var properties = {}
	for mod in playerModuleProps:
...
func clear_player_modules():
	playerModuleProps = {}
	for child in vflow.get_children():
		if child.is_in_group(pl_set_g):
			vflow.remove_child(child)

func add_player_module(path:String, mname:String, properties:Dictionary):
...
func update_map_list(gs):
	var mapdir = DirAccess.open(Global.maps_dir)
	mapPaths_b.clear()
	var id = 0
	for map in mapdir.get_files():
...
func _on_snake_speed_drag_ended(_value_changed):
	_on_settings_changed()

func _on_start_snake_length_drag_ended(_value_changed):
	_on_settings_changed()

func _on_map_paths_b_item_selected(_index):
	_on_settings_changed()
