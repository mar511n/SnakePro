...
func on_collected_by_player(player:SnakePlayer)->bool:
	if !player.module_vars["PlayerIsAlive"]:
		...
		is_used = true
		Global.Print("revive player %s and mark ReviveItem for removal" % player.peer_id)
...
func on_player_reset_snake_tiles():
	if !is_used and !pl.module_vars["PlayerIsAlive"]:
		pl.ReviveSound.play()
		is_used = true
		Global.Print("dont let player %s die and mark ReviveItem for removal" % pl.peer_id)
